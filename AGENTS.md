...
## ARIA Graphify Publication Boundary

Graphify remains derived navigation, never a project-truth owner. Use
`aria-nbv-context` for ARIA eligibility, projection, freshness, and exact-source
validation. Only the primary integration checkout publishes the tracked
snapshot; linked worktrees query a usable committed baseline, validate reported
stale sources directly, and do not publish Graphify output.

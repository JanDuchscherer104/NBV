    ...
    *,
    ...
    if top_rows:
        ...
        top_fig.update_layout(
            title=(
                "Top-k valid candidate "
                f"{current_scientific_label('target_root_gain')} / {current_scientific_label('target_rri')} per step"
            ),
            xaxis_title="rollout step",
            yaxis_title=(f"{current_scientific_label('target_root_gain')} / {current_scientific_label('target_rri')}"),
        )
    ...
    if rows_df["J_endpoint"].notna().any() or rows_df["log_gain"].notna().any():
        ...
        endpoint_fig.add_trace(
            go.Bar(
                x=rows_df["trajectory"],
                y=rows_df["J_endpoint"],
                name=current_scientific_label("endpoint_gain"),
            )
        )
        endpoint_fig.add_trace(
            go.Bar(x=rows_df["trajectory"], y=rows_df["log_gain"], name=current_scientific_label("log_gain"))
        )

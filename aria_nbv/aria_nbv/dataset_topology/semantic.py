...
from typing import Any, Literal, Protocol, SupportsInt, cast
...
class DatasetTopologyNode:
    ...
    def status(self) -> TopologyStatus:
        """Expose semantic availability through the shared status vocabulary."""
...
class DatasetTopologyEdge:
    ...
    @property
    def status(self) -> TopologyStatus:
        """Expose semantic resolution through the shared status vocabulary."""
...
class DatasetTopology:
    ...
    def tree_text(self) -> str:
        """Return the compatibility Rich projection through the shared contract."""

        return self.plain_text_tree()

    def diagram_dot(self) -> None:
        """Semantic snapshots currently use Sankey data instead of Graphviz."""
...
def build_dataset_topology(
    *,
    ...
    paths = cast("_TopologyPaths", path_config if path_config is not None else _DefaultTopologyPaths())
...
def _add_rollout_payload_nodes(
    ...
    *,
    ...
    root_attrs = _string_object_dict(manifest.get("root_attrs"))
    counts = _string_object_dict(manifest.get("counts"))
...
def _add_known_artifacts(
    ...
    *,
    ...
    artifact_paths: list[Path | None] = list(paths) if paths else [None]
...
def _string_object_dict(value: object) -> dict[str, Any]:
    """Narrow a JSON-like mapping to the string-keyed object shape used here."""

...
def test_corrected_v7_pilot_has_fresh_identity_and_unchanged_paired_contract():
    config = CudaRolloutCampaignConfig.from_toml(
        REPO_ROOT / ".configs/build_rollouts_v1_cuda_campaign_pilot_corrected_v7.toml"
    )
    assert config.campaign_id == "cuda-rollouts-v1-pilot-corrected-v7"
    assert not (REPO_ROOT / ".configs/build_rollouts_v1_cuda_campaign_pilot_corrected.toml").exists()
    assert config.output_root == Path(".campaign/cuda-rollouts-v1-pilot-corrected-v7")

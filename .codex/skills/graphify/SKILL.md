...
Before traversal, expand the question against the graph's own vocabulary so a wording mismatch does not collapse the answer to noise. If the CLI is unavailable or freshness fails, inspect exact source owners instead of maintaining a local traversal implementation. Answer using only graph output or exact source evidence, and quote `source_location` when citing a graph fact. For query expansion, traversal flags, and the `/graphify path` and `/graphify explain` flows, see `references/query.md`.

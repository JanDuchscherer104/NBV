...
#let aria-notation-symbols = (
  ...
  (key: "rl.history", tex: "\\mathcal{H}_t", description: "Actor-visible observation and action history available before decision step t.", thesis_list: true, order: 241),
  (key: "rl.decision_protocol", tex: "\\Xi", description: "Frozen decision protocol comprising generator, target source, state map, action-support provenance, reward, discount, and horizon.", thesis_list: true, order: 242),
  (key: "rl.representation", tex: "z_t^{\\sigma}", description: "Representation produced from actor-visible history by state-construction protocol sigma.", thesis_list: true, order: 243),
  (key: "rl.representation_map", tex: "\\phi_{\\sigma}", description: "State-construction map from actor-visible history to the scorer representation.", thesis_list: true, order: 244),
  ...
  (key: "rl.s_surface", tex: "s_t^{\\mathrm{S1-surface}}", description: "Implemented privileged selected-surface control state; it consumes only previously selected rendered depth and is not deployable.", thesis_list: true, order: 302),
  (key: "rl.s_ray", tex: "s_t^{\\mathrm{S2-ray}}", description: "Planned actor-visible ray-aware state preserving observed surface, free, unknown, support, uncertainty, source, and recency.", thesis_list: true, order: 303),
  ...
  (key: "rl.selected_pose_prefix", tex: "\\boldsymbol{H}_t^{\\mathrm{pose}}", description: "Strictly causal prefix of previously selected poses available at decision step t.", thesis_list: true, order: 321),
  (key: "rl.selected_pose_prefix_next", tex: "\\boldsymbol{H}_{t+1}^{\\mathrm{pose}}", description: "Strictly causal selected-pose prefix after applying the selected transition.", thesis_list: false, order: 322),
  ...
  (key: "rl.learned_q", tex: "\\widehat{Q}_{\\theta,h}^{\\sigma,\\Xi}", description: "Learned representation- and protocol-conditioned finite-horizon value predictor.", thesis_list: true, order: 356),
  (key: "rl.support_score", tex: "U_{\\theta,h}^{\\Xi}", description: "Support-conditioned candidate-ranking score reserved for candidate-set interaction models.", thesis_list: true, order: 357),
  ...
  (key: "model.target_token", tex: "\\boldsymbol{h}_e^{\\mathrm{tgt}}", description: "Learned selected-target token built from root-relative target pose and metric extents.", thesis_list: true, order: 600),
  (key: "model.candidate_physical_token", tex: "\\boldsymbol{h}_{t,i}^{\\mathrm{phys}}", description: "Candidate-local physical token from root/current-relative candidate poses and compact root evidence.", thesis_list: true, order: 605),
  (key: "model.candidate_row", tex: "\\boldsymbol{x}_{t,i}", description: "Candidate-local conditional-value query formed by adding candidate-from-target pose encoding to the physical token.", thesis_list: true, order: 610),
  ...
  (key: "rl.action_set_t", tex: "\\mathcal{A}_t", description: "Finite feasible action set at rollout step t.", thesis_list: true, order: 725),

...
#let aria-notation-equations = (
  ...
  (key: "model.qh_history_controls", tex: "\\boldsymbol{p}_{t,j}^{\\mathrm{hist}}=\\operatorname{PoseEnc}(T_{c_t\\leftarrow c_j}),\\ a_{t,j}^{\\mathrm{hist}}=(t-1-j)/H_{\\max};\\quad \\boldsymbol{h}_t^{\\mathrm{H0}}=\\operatorname{HistProj}(\\operatorname{Mean}_{j<t}\\boldsymbol{p}_{t,j}^{\\mathrm{hist}}),\\quad \\boldsymbol{h}_t^{\\mathrm{H1}}=\\operatorname{HistProj}(\\operatorname{LastValid}(\\operatorname{CausalTransformer}([\\boldsymbol{e}_{\\emptyset},\\{\\boldsymbol{p}_{t,j}^{\\mathrm{hist}}+g(a_{t,j}^{\\mathrm{hist}})\\}_{j<t}])))", description: "Checkpoint-compatible masked-mean H0 and exploratory ordered causal-transformer H1 selected-pose history controls.", thesis_list: false, order: 9999),
  (key: "model.qh_cfplus_h0_control", tex: "\\operatorname{Struct}(\\boldsymbol{o}_t)=\\operatorname{Struct}(\\boldsymbol{o}'_t)\\Rightarrow f_\\theta^{\\mathrm{CF{+}-H0}}(s_t^{\\mathrm{S0-pose}},\\boldsymbol{o}_t,\\boldsymbol{\\phi}_e,\\{q_{t,i}\\}_{i=1}^{N_q},h)=f_\\theta^{\\mathrm{CF{+}-H0}}(s_t^{\\mathrm{S0-pose}},\\boldsymbol{o}'_t,\\boldsymbol{\\phi}_e,\\{q_{t,i}\\}_{i=1}^{N_q},h)", description: "Source-matched CF+ H0 validates selected-observation structure while remaining exactly invariant to its numeric payload.", thesis_list: false, order: 9999),
  (key: "model.qh_s1_selected_surface", tex: "\\boldsymbol{p}_{t,j,u}^{c_t}=T_{c_t\\leftarrow r}T_{r\\leftarrow c_j}\\pi^{-1}(u,D_{j,u}^{\\mathrm{sel}}),\\ j<t;\\quad \\boldsymbol{z}_{t,j,u}=\\phi_{\\mathrm{pt}}(\\boldsymbol{p}_{t,j,u}^{c_t}/\\sigma_{\\mathrm{xyz}});\\quad \\boldsymbol{g}_t^{\\mathrm{S1}}=[\\operatorname{Mean}\\boldsymbol{z},\\operatorname{Max}\\boldsymbol{z},\\rho_t^{\\mathrm{present}},\\rho_t^{\\mathrm{pixel}},\\rho_t^{\\mathrm{view}}];\\quad \\boldsymbol{\\Phi}_t^{\\mathrm{S1}}=\\boldsymbol{\\Phi}_t^{\\mathrm{root}}+W_{\\mathrm{pt}}\\boldsymbol{g}_t^{\\mathrm{S1}},\\quad W_{\\mathrm{pt}}^{(0)}=0", description: "Fixed-width identity-start privileged S1 selected-surface residual over current-camera point sets.", thesis_list: false, order: 9999),

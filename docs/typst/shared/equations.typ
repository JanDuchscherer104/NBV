...
#let aria-notation-equations = (
  ...
  (key: "rl.candidate_row_equivariance", tex: "f_\\theta(\\Pi X_t,\\Pi m_t^{\\mathrm{cand}})=\\Pi f_\\theta(X_t,m_t^{\\mathrm{cand}})", description: "", thesis_list: false, order: 9999),
  (key: "rl.masked_candidate_selection", tex: "\\mathcal{A}_t=\\{i:m_{t,i}^{\\mathrm{act}}=1\\},\\quad a_t^\\theta=\\operatorname*{argmax}_{i\\in\\mathcal{A}_t} Q_{h,\\theta,e}(s_t,i)", description: "", thesis_list: false, order: 9999),
  ...
  (key: "rl.finite_horizon_return", tex: "G_{t,e}^{(h)}=\\sum_{k=0}^{h-1}\\gamma^k r_{t+k}^e", description: "Admissible finite-horizon return; post-terminal rewards are zero under the absorbing-terminal convention.", thesis_list: false, order: 9999),
  (key: "rl.q_h", tex: "Q_{h,e}^{\\star}(s_t,i)=\\sup_{\\pi\\in\\Pi^{\\mathrm{act}}}\\mathbb{E}_{\\pi}\\left[G_{t,e}^{(h)}\\mid s_t,a_t=i\\right],\\quad 1\\le h\\le b_t\\le H_{\\max},\\quad Q_{0,e}^{\\star}=0", description: "Optimal continuation value within the generated finite candidate sets and hard-action support.", thesis_list: false, order: 9999),
  ...
  (key: "model.qh_candidate_state_cross_attention", tex: "\\boldsymbol{u}_{t,i}=\\operatorname{CrossAttn}_{\\theta}(\\boldsymbol{x}_{t,i},\\{\\boldsymbol{h}_e^{\\mathrm{tgt}},\\boldsymbol{\\Phi}_t^{\\mathrm{scene}},\\boldsymbol{H}_t,\\operatorname{Emb}(t),\\operatorname{Emb}(H),\\boldsymbol{b}_t\\})", description: "", thesis_list: false, order: 9999),
  (key: "rl.candidate_mask_isolation", tex: "\\operatorname{Mask}(\\boldsymbol{X}_t,\\boldsymbol{m}_t^{\\mathrm{cand}})=\\operatorname{Mask}(\\widetilde{\\boldsymbol{X}}_t,\\boldsymbol{m}_t^{\\mathrm{cand}})", description: "", thesis_list: false, order: 9999),
  ...
  (key: "rl.qh_coral_interface", tex: "\\begin{gathered}p_{t,i,k}^{\\mathrm{CORAL}}=\\sigma(o_{t,i,k}^{\\mathrm{CORAL}}),\\quad k=0,\\ldots,K-2;\\\\ \\pi_{t,i,k}^{\\mathrm{CORAL}}=p_{t,i,k-1}^{\\mathrm{CORAL}}-p_{t,i,k}^{\\mathrm{CORAL}},\\quad p_{t,i,-1}^{\\mathrm{CORAL}}=1,\\quad p_{t,i,K-1}^{\\mathrm{CORAL}}=0;\\\\ \\hat{r}_\\psi^e(s_t^{\\mathrm{cf0}},\\boldsymbol{\\phi}_e,q_{t,i})=\\sum_{k=0}^{K-1}\\pi_{t,i,k}^{\\mathrm{CORAL}}u_k\\end{gathered}", description: "", thesis_list: false, order: 9999),
  (key: "rl.qh_masked_argmax", tex: "a_t^\\theta=\\operatorname*{argmax}_{i:m_{t,i}^{\\mathrm{act}}=1}Q_{H,\\theta,i}", description: "", thesis_list: false, order: 9999),
  (key: "rl.qh_supported_successor_set", tex: "\\mathcal{A}_{t+1}^{Q,h-1}=\\{i:m_{t+1,i}^{\\mathrm{act}}=1\\land m_{t+1,i}^{Q,h-1}=1\\}", description: "Hard-valid successor candidates with factual value support at horizon h-1.", thesis_list: false, order: 9999),
  (key: "rl.qh_doubleq_index", tex: "B_t^{(h,e)}=Q_{h-1,\\theta^-}(s_{t+1},\\operatorname*{argmax}_{i\\in\\mathcal{A}_{t+1}^{Q,h-1}}Q_{h-1,\\theta}(s_{t+1},i))", description: "Successor maximum over the certified horizon-specific support set; terminal continuation is zero.", thesis_list: false, order: 9999),

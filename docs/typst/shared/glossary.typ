...
#let aria-glossary-entries = (
  ...
  (
    ...
    description: "Target-conditioned finite-candidate value function trained from ASE oracle rollout traces. Conditional Q fixes the candidate selected first, target, scalar requested horizon, and named continuation rule; it remains separate from feasibility and action masking. The implemented feature-matched A0/A1-S0-pose/root-moments controls emit this value and a separate feasibility signal for each materialized candidate; A1 remains the default and adapters apply hard validity masks for training and selection.",
    group: "Model",
    custom: (

...
ARIA-NBV sits between classical view planning, learned reconstruction-quality scoring, and egocentric scene understanding @ActivePerception-bajcsy1988 @ActiveVision-aloimonos1988. Classical @next-best-view work formalizes the generate-score-select loop for three-dimensional inspection and reconstruction @NBVSystem-banta2000 @RecedingHorizonNBV-bircher2016 @ShadowcastingNBV-batinovic2022, while submodular and adaptive-submodular active-sensing results explain why greedy policies can be strong when the utility has diminishing returns @NemhauserSubmodular1978 @SubmodularNBV-lauri2020 @ViewPlanningSurvey-scott2003 @KrauseSensorPlacement2008 @AdaptiveSubmodularity-golovin2011. This thesis uses that lineage as a baseline discipline rather than as a theorem claim: target-specific @relative-reconstruction-improvement depends on masks, object matching, candidate regeneration, occlusion, and changing support, so coverage-style approximation guarantees are treated as motivation for greedy controls, not as properties of the ARIA objective.
...
*Project Aria, @aria-synthetic-environments, and @egocentric-voxel-lifting.* This stack defines the actor-visible substrate through egocentric sensing, calibration, semi-dense support, local voxel evidence, and OBB hypotheses. Its boundary is equally important: meshes and target crops remain supervision and evaluation tools, not part of the online actor-visible state.

*Coverage, uncertainty, and information-gain methods.* SCONE, MACARONS, FisherRF, and related radiance-field or Gaussian-splatting approaches contribute support, overlap, uncertainty, information-gain, and directional-novelty signals. In this thesis those quantities are diagnostic channels or candidate-token features, not replacements for @target-specific-rri as the primary objective.

*Set and geometric models.* Deep Sets, Set Transformer, and geometric-learning arguments provide the structural prior for finite candidate tables: permutation-aware scoring, masked attention, local relative encodings, and explicit symmetry tests. ARIA-NBV uses this family to shape #symb.rl.qh controls before escalating to heavier point or sparse backbones.

*Continuous, hierarchical, and recurrent bridges.* GenNBV, Hestia, and Deja View motivate later bridge experiments in continuous control, hierarchical decomposition, or bounded refinement. They remain downstream ablations until finite-candidate labels, invalid-action masks, and replay-based evaluation are trustworthy.

...
ARIA-NBV is formulated as target-conditioned selection from a finite candidate table. The implemented substrate generates and evaluates masked multi-step pose rollouts, records selected transitions, exposes a dense training view, and trains an actor-only #symb.rl.qh scorer through fitted Double-Q optimization. The implemented `A1--S0-pose--root-moments` control independently queries shared scene, target, causal-history, budget, and horizon context for every materialized candidate. It is an executable architecture baseline, not yet a policy result or evidence that the compact state is task-sufficient.

The frozen interface distinguishes remaining budget $b_t$, scalar requested residual horizon $h$, and configured support bound $H_"max"$. Omitting $h$ requests $h=b_t$; realized queries require $1 <= h <= b_t <= H_"max"$, and padding uses zero. The scorer returns action-mask-independent conditional values and feasibility logits, while Lightning and online inference retain hard-mask ownership. Direct continuous Huber regression is the canonical value objective; exact horizon-two targets, recursive lower-horizon backups, Double-Q selection/evaluation, CORAL auxiliaries, and behavior-policy returns remain separately named controls or ablations.

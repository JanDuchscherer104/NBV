...
The minimal learned-method goal is one continuous candidate scorer $Q_theta(s_t,e,i,h)$ that shares scene, target, and candidate encoders while accepting the requested residual horizon $h$ explicitly. The H=2 tracer is a base-case implementation rather than the target architecture. Dense $Q_1$, exact $Q_2$, horizon-recursive fitted value learning, optional Double-Q max correction, and behavior-policy return regression are treated as distinct learning targets or controls.

The chapter first fixes the actor-visible state and the coverage limitation of local EFM3D evidence. It then specifies persisted and model-facing DTO roles, the finite-action and replay semantics, geometric and variable-horizon acceptance tests, orthogonal scene-carrier and interaction-architecture ladders, and the bounded #symb.rl.qh learning targets. Descriptive status blocks distinguish implementation maturity from evidence maturity throughout.

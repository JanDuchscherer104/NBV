...
)[The canonical state separates immutable root context from causal dynamic memory. A bounded, privileged S1 selected-surface residual is executable behind the scene-carrier seam; promotion still requires matched held-out evidence, and S2 free/unknown memory remains planned.]
...
#figure(
  text(size: 8.2pt, table(
    ...
    [`S1-points`], [causally fused selected surface points], [Implemented privileged identity-start, fixed-width residual over current-camera point sets; does not distinguish observed free from unknown space.],
...
#figure(
  text(size: 8.2pt, table(
    ...
    [Selected-surface point memory], [implemented privileged identity-start control; unpromoted], [Strictly causal surfaces follow selected views; fixed width and a zero-output residual preserve matched H0 initialization.], [No free/unknown or ray identity; test density, source dropout, runtime, and held-out value before candidate-local refinement.],

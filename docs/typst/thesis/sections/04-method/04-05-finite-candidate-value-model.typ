...
#thesis_status(
  ...
  source: "aria_nbv/aria_nbv/data_handling/qh_contracts.py; aria_nbv/aria_nbv/vin/models/target_finite_horizon.py; aria_nbv/aria_nbv/lightning/qh_module.py; aria_nbv/aria_nbv/lightning/qh_q2_certification.py; aria_nbv/aria_nbv/oracle/pipelines/online_qh.py; aria_nbv/tests/lightning/test_qh_q2_certification.py",
...
)[The modular A0/A1 finite-horizon scorer, H0/H1 pose-history seam, S0 root-moments and privileged S1 selected-surface scene carriers, typed output, scalar horizon query, feasibility auxiliary, regression/CORAL value decoders, fitted-Q learner, bounded exact-Q2 certifier, hard-masked online adapter, and dense-valid data path are implemented. Both decoders complete a one-epoch GPU fit on real bounded CF0 and source-matched CF+ train/validation corpora. A new five-scene CF+ test population retains causal selected depth and exactly matches the earlier candidate and rollout identities. In three matched 100-update regression seeds, S1 has lower median but higher mean and variance for held-out fitted-Q loss than H0, and worse mean exact-Q2 error. This evidence rejects S1 promotion while motivating a separately tested identity-start residual. Neither CF+ profile is publishable as an inference bundle. A1 remains the default interaction and H0 the default history control; H1 and S1 are exploratory. The learned exact-Q2 and scientific policy gates remain unmet.]

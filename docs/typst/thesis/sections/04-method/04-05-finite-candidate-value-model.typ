...
)[The modular A0/A1 finite-horizon scorer, H0/H1 pose-history seam, S0 root-moments and privileged S1 selected-surface scene carriers, typed output, scalar horizon query, feasibility auxiliary, regression/CORAL value decoders, fitted-Q learner, bounded exact-Q2 certifier, hard-masked online adapter, and dense-valid data path are implemented. Both decoders complete a one-epoch GPU fit on a real bounded CF0 corpus. Regression and CORAL also complete direct one-epoch GPU fits for both the source-matched CF+ H0 control and S1 on the same bounded selected-depth train/validation stores. These CF+ results establish executable optimization only: neither profile is publishable as an inference bundle and the stores provide no held-out S1 evidence. A1 remains the default interaction and H0 the default history control; H1 and S1 are exploratory. The learned exact-Q2 and scientific policy gates remain unmet.]
...
The metric divisor $sigma_"xyz"$, pixel stride, view-chunk bound, point width, raster geometry, and source profile are experiment identity. $rho^"pixel"$ is valid sampled pixels divided by causal sampled-pixel capacity; $rho^"view"$ is selected observations with any valid point divided by causal observation count. An empty or all-invalid prefix yields exactly the root-moments control. Mean pooling makes the carrier density weighted rather than duplicate invariant. It retains neither source-view identity nor ray/free/unknown semantics, and it is target- and candidate-independent. Candidate-relative point attention and Déjà-View-style recurrent refinement remain later, separately promoted interactions.

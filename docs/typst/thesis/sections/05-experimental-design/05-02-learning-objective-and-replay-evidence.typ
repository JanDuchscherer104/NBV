...
Prediction support and loss support are therefore different. The scorer may emit finite values for every materialized candidate. Dense one-step loss uses every hard-valid candidate with a finite immediate label. For $h>1$, the present replay usually has a factual successor only for the selected action, so recursive loss is applied only to that selected state--action transition. The padded candidate dimension must not be read as dense multi-step supervision, and an unselected candidate receives no invented successor or return.

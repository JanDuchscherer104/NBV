...
#figure(
  align(center, image(
    ...
    width: 100%,
    alt: "A dominant oblique 3D view shows a neutral processed ground-truth room mesh, an orange double-outline task bounding box, a solid magenta arrowed physical RGB trajectory ending at a hollow current-pose ring, two violet dashed calibrated historical frusta with hollow pose centers, a slightly offset black sampling-root point nested inside the larger endpoint ring, and the black-and-gold selected candidate frustum. A narrow bird's-eye audit shows all sixty candidate centers and validity decisions. " + candidate-pose-display + " " + candidate-family-display,
  )),
  caption: [Scene grounding and support audit for one pinned ASE decision state. Panel A overlays distinct semantic layers on a neutral processed ASE GT mesh: the orange double outline is the task GT OBB; the solid arrow ends at the hollow ring for the current physical RGB pose; dashed violet wireframes with hollow centres mark two logged RGB cameras; and the black frustum with a gold-backed route denotes selected shell 47. The nested filled point is the near-coincident but frame-distinct canonical sampling root. #candidate-pose-display Panel B retains all 60 stored centers and validity decisions: 25 admissible, 35 hard-rejected, and shell 47 selected. #candidate-family-display Frusta approximate the calibrated Fisheye624 valid domain; the example fixes support and validity, not policy performance.],
...
#figure(
  ...
  alt: "A three-panel controlled metric fixture. Panel A shows a reconstructed point and its exact closest point on a triangle. Panel B uses a solid arrow from a reconstructed point to its nearest triangle and a dashed arrow from a triangle to its nearest reconstructed point, distinguishing the two directional reductions. Panel C holds one planar surface and five points fixed while replacing four equal-area faces with a left-weighted non-uniform tessellation in which the left region contributes thirty-two of forty equal-weight faces. Accuracy remains 0.00640 square metres, while equal-face completeness changes from 0.03640 to 0.02284 square metres. The values are synthetic validity evidence, not an ASE result.",
  caption: [Controlled point--mesh validity fixture computed with the repository metric and exact witnesses. The construction isolates the asymmetric reductions and equal-face tessellation sensitivity while holding surface support and points fixed. It provides synthetic mechanism evidence, not an ASE performance result.],

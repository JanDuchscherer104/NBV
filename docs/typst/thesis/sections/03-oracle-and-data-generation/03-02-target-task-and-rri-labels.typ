...
The implemented sampler never starts from actor proposals. From a multi-slice @ground-truth:short OBB block, it selects the latest slice containing at least one non-padding row; if no such slice exists, the final all-padding slice yields no task rows. Within the selected slice, it removes padding and attempts to construct one target descriptor per remaining OBB while preserving source indices and confidence. Descriptor construction requires finite poses and extents and strictly positive extents. Because this validation occurs before row append, any malformed OBB raises an error before the sampler can return a result; invalid geometry is not retained as an audit row. A successful `rows` table is therefore identical to the geometry-valid target-row set $cal(R)_s^"geom"$. Seeded uniform sampling without replacement then selects at most the configured per-snippet cap. The resulting `matched` status means that the @ground-truth:short row passed this geometry contract, not that a proposal was associated with an identity. Confidence, IoU, ambiguity-gap, visibility, support, headroom, and utility do not gate the successful sampling path.
...
#figure(
  ...
  alt: "A left-to-right population flow begins after selecting the latest ground-truth OBB slice that contains a non-padding row; an all-padding block produces no task rows. Padding is removed to obtain source rows. Descriptor validation requires finite poses and extents and strictly positive extents. All valid examples enter the rows table; any invalid example takes a fail-fast branch and raises before a sampler result. Seeded uniform sampling without replacement, capped by the smaller of K and the validated-row count, emits selected rows with provenance. Actor proposals and utility evidence do not gate the successful sampling path; later scorer-evidence checks remain outside the depicted sampler.",
  caption: [Oracle target-task sampler. After latest-slice selection and padding removal, every source row must satisfy the target-descriptor geometry contract; one invalid row aborts construction rather than entering an audit table. A successful `rows` result equals $cal(R)_s^"geom"$, from which the seeded cap emits selected tasks. This privileged path neither matches actor observations nor evaluates task utility.],
) <fig:oracle-target-task-sampler-contract>

This fail-fast boundary matters for interpretation: the current store cannot estimate an invalid-target rate from sampler rows because malformed geometry raises before a sampling result is returned. Downstream scoring may separately invalidate a selected task when its mesh crop, current support, or rendered evidence is unusable. Near-solved and negative-gain tasks otherwise remain scientifically informative because sampler membership encodes neither headroom nor utility. A later actor-visible protocol must introduce proposal identity and observation-quality diagnostics rather than reinterpret these oracle fields as measurements.

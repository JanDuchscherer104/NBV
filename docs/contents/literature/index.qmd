...
| {{< gls three-dimensional-gaussian-splatting >}} / radiance-field active reconstruction | [Active 3DGS and targeted NBV](active_3dgs_nbv.qmd), [Instance-aware object-centric 3DGS NBV](3dgs_instance_nbv.qmd), ActiveNeRF, FisherRF, Next Best Sense, dynamic 3DGS, FOV-HPE | proposal/diagnostic and stretch/bridge | Uncertainty, Fisher information, per-Gaussian object state, target/object weighting, downstream-task view selection | Treating 3DGS uncertainty/logdet or human-pose reward as a substitute for ASE mesh-supervised target RRI |
...
- [Instance-aware object-centric 3DGS NBV](3dgs_instance_nbv.qmd): ATEK-focused distillation of per-Gaussian instance state, confidence-weighted block information, target gating, the paper's strongest evidence, and the actor/oracle-safe adoption path [@ObjectCentricNBV-jeong2026].
...
flowchart LR
  ...
  B -. "optional analytic features" .-> H["Object-weighted Gaussian information"]
  H --> D["One-step candidate scorer"]
  C --> D
...
The thesis should first prove that deterministic bounded oracle lookahead improves cumulative {{< gls target-specific-rri >}} over one-step greedy under equal acquisition budget. It should then train a target-conditioned fitted Double-Q / {{< gls finite-horizon-q-function >}} model over finite candidate sets and require it to beat one-step greedy/model scoring on cumulative target RRI. Object-weighted 3DGS information may be added as an auxiliary candidate feature only after one-step calibration against realized target RRI. IQL, actor-critic bridges, SB3/PPO/SAC, Habitat/Isaac, and 3DGS control remain gated follow-up or stretch work.
...
- [`figures/external/arXiv/Instance-NBV/`](../../figures/external/arXiv/Instance-NBV/): three selected source PNGs used by the ATEK-focused Instance-NBV review.
...
Return to [main documentation](../../index.qmd), [research questions](../../typst/thesis/sections/01-research-questions.typ), [roadmap](../../typst/thesis/development/roadmap.typ), or [RRI theory](../theory/rri_theory.qmd).

...
The reported coefficients are \(\beta_d=10\) and \(\beta_o=1\).
...
Despite the adaptive normalization, the final utility uses \(\beta_d=10\) and \(\beta_o=1\). This is a task-dependent scalarization. The paper does not report sensitivity, transfer across scenes, or a Pareto analysis.
...
where \(v^{target}\) is predicted target visibility/support, \(h^{obj}\) is object-posterior entropy, \(u^{opacity}\) is an opacity-deficit summary, \(m\) is validity, \(\rho\) is an invalid/low-support reason, and \(\Delta T\) is target/current-pose-relative geometry.

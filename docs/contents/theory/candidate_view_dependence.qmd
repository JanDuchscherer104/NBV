...
| Set Transformer [@SetTransformer-lee2019] | Attention can model interactions among set elements while preserving set symmetry. | Masked candidate self-attention is the default interaction test. |
...
Use an independent calibrated base plus a zero-mean set residual. This preserves absolute RRI semantics while allowing set context to alter relative ranking:
...
\delta_i
=
f_{\mathrm{set}}(x_i,\{x_j\}_{j=1}^{N_q},m_t),
\qquad
\bar\delta
=
\frac{1}{|\mathcal A_t|}
\sum_{j\in\mathcal A_t}\delta_j,
$$
...
\hat r_i^{\mathrm{abs}}
+
\lambda(\delta_i-\bar\delta)
}.
$$

For $Q_H$, the same pattern becomes the residual dueling value head:
...
-
\frac{1}{|\mathcal A_t|}
\sum_{j\in\mathcal A_t} A_{\theta,j}^{H}.
$$

The mean is taken only over valid candidate rows.
...
| A1 | DeepSets context $g=\rho(\sum_{j\in\mathcal A_t}\phi(x_j))$, $\hat r_i=f(x_i,g)$ | Test whether global candidate-set context helps. |
| A2 | Masked Set Transformer $Z=\mathrm{SetTransformer}(X,m_t)$, $\hat r_i=h(Z_i)$ | Test candidate-candidate interaction. |
| A3 | Set Transformer plus $\mathrm{SE}(3)$ relative bias | Test geometric interaction. |
| A4 | Fisher/SCONE overlap bias $o_{ij}^{\mathrm{target}},o_{ij}^{\mathrm{frustum}},o_{ij}^{\mathrm{dir}}$ | Test information-overlap structure. |
| A5 | Residual dueling $Q_H$ over finite candidates | Main finite-horizon use case. |
...
|---|---|---|
| Absolute-label contamination | The same candidate receives different predicted immediate RRI depending on unrelated sampled rows. | Keep an independent absolute head and constrain set context to zero-mean residuals. |
...
> The independent VIN-style scorer estimates point samples of a view-utility field over $\mathrm{SE}(3)$. ARIA-NBV tests whether a permutation-equivariant candidate-set model can exploit the local geometric and information-overlap structure of the sampled candidate population to improve candidate ranking and finite-horizon value prediction.

This is grounded by VIN-NBV's RRI-based sampled-view ranking [@VIN-NBV-frahm2025], FisherRF's expected-information-gain view of candidate measurements [@FisherRF-jiang2024], Deep Sets and Set Transformer set modeling [@DeepSets-zaheer2017; @SetTransformer-lee2019], and geometric deep learning's symmetry principle [@GeometricDeepLearning-bronstein2021]. The implementation should stay conservative: retain the calibrated independent scorer and let candidate-set interaction act as a residual advantage correction unless experiments justify a stronger coupling.

...
[`evidence.json`](candidate_generation_scaleup_pilot/evidence.json). It links
the three profile metrics, the family funnel, the evaluator identity, and the
plot provenance. The interactive candidate-centre, target-framing, view-jitter,
support, and family-funnel views are generated from the same persisted audit
rows; they are intended for exploratory inspection, not as independent
measurements. In particular, a candidate inside a geometric shell is not proof
of line of sight, collision-free execution, depth-rendering fidelity, or visual
realism.

The diagnostic persistence and inspection seam under review in
...
- [Project Aria MPS](https://facebookresearch.github.io/projectaria_tools/docs/ARK/mps)
  motivates preserving calibrated trajectories and semi-dense observed
  support as actor-visible evidence, rather than using GT mesh geometry as an
...
1. **Family-aware bounded reservoir and refill.** Draw a bounded proposal
   ...
   family gate in [#54](https://github.com/JanDuchscherer104/ARIA-NBV/issues/54).
...
- The negative challenger artifacts remain part of the evidence record. Their
  rejection is specifically due to the family-collapse gate, not evidence that
  target glances or radius strata can never be useful after support repair.

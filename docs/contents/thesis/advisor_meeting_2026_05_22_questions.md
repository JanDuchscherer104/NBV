...
2. Should the first representation pass prioritize compressed DINO-on-semidense-point tokens over EVL voxel/neck/head features because the EVL field is local and smaller than the semidense evidence volume?
...
8. What candidate-row interaction checks should gate \(Q_H\): independent scorer, pooled DeepSets, zero-mean residual, masked Set Transformer, row-shuffle invariance, duplicate-row tests, and mask-isolation tests?
...
12. Which qualitative failures should be shown in Rerun for advisor review: flat reward fields, invalid candidate collapse, target ambiguity, poor DINO/semidense support, or Q policy overfitting to candidate family provenance?

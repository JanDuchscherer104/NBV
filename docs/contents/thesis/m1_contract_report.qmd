...
|---|---|---|
| Offline store layout | Pass for source contract | Immutable format is owned by `aria_nbv/aria_nbv/data_handling/vin_store/store.py`, `vin_store/format.py`, `vin_store/writer.py`, and `vin_store/dataset.py`. |
| Split contract | Pass for source contract | `splits/*.npy` contain global sample indices; `assign_offline_splits` uses stable sample-key hashing and preserves sample-index order inside each split. |
...
| Rollout replay alignment | Pass for synthetic guard | The historical wide trace DTO was superseded by pipeline-local `EvaluatedRolloutRecord` plus factual `steps/` and `candidates/` tables; invalid rows remain masked with false action/train masks and `NaN` labels. |
...
| Current local store evidence | Blocked for M1 exit | `.configs/training/vin/offline_only.toml` resolves to `.data/offline_cache/vin_offline`; the evidence snapshot below records manifest v7, 48 sample-index rows, and a partial one-scene diagnostic store. |
...
|---|---|---|
| Config source | Pass for local smoke | `.configs/training/vin/offline_only.toml` uses `datamodule_config.source.kind = "offline"` and `store_dir = "vin_offline"`. |
...
| Manifest version | `OFFLINE_DATASET_VERSION` in `data_handling/vin_store/store.py` | `VinOfflineStoreReader` rejects unsupported versions and asks for a rebuild. |
| Sample index | `VinOfflineIndexRecord` in `data_handling/vin_store/format.py` | Rows carry global `sample_index`, `sample_key`, `scene_id`, `snippet_id`, split, shard id, and shard-local row. |
| Splits | `assign_offline_splits()` and `VinOfflineStoreReader.get_split_records()` | Split arrays reference global sample indices; selected records are returned in split-array order. |
...
| Rollout replay | `EvaluatedRolloutRecord` at the Oracle pipeline edge and `rollouts.zarr` factual tables in `aria_nbv.rollouts` | Actor-visible full-shell pose/validity fields stay separate from oracle-only score and metric vectors. |
...
```sh
cd aria_nbv
uv run nbv-rerun-inspect --config-path ../.configs/inspection/rerun/rerun_offline.toml --split val --index 0 --save ../.artifacts/rerun/m1-val-000.rrd
```

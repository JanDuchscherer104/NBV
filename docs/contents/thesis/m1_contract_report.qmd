...
| Manifest version | `OFFLINE_DATASET_VERSION` in `data_handling/offline/store.py` | `VinOfflineStoreReader` rejects unsupported versions and asks for a rebuild. |
| Sample index | `VinOfflineIndexRecord` in `data_handling/offline/format.py` | Rows carry global `sample_index`, `sample_key`, `scene_id`, `snippet_id`, split, shard id, and shard-local row. |

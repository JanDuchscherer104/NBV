...
## CUDA, Mojo, and CPU Expectations {#cuda-and-cpu-expectations}

Apple Silicon machines use the native PyTorch3D Mojo backend for the
point--mesh distance and mesh-rasterization forward calls required by pose
generation, depth rendering, VIN-store generation, and multi-step rollouts. Set
`PYTORCH3D_BACKEND=mojo` to require that path. See the
[PyTorch3D Mojo backend guide](mojo_pytorch3d.qmd) for its supported contracts,
setup, and verification report.

Apple Silicon with Mojo and Linux with CUDA are supported data-generation
paths. The package pins PyTorch CUDA 12.1 wheels on Linux, and several configs
use `device = "auto"`. Set `PYTORCH3D_BACKEND=cuda` to require CUDA geometry. A
compiled CPU backend remains available for reference comparisons, docs work,
config validation, lightweight tests, immutable-store reads, and dry runs; it
is not the performance target for full data generation.

...
```sh
cd aria_nbv
uv run nbv-build-rollouts --config-path ../.configs/generation/rollouts/benchmarks/build_rollouts_qh_v0_baseline.toml
```
...
python scripts/qh_loader_benchmark.py \
  --config .configs/evidence/qh/qh_loader_benchmark.json \

...
**Snapshot:** 2026-08-24, repository `52e9d262577260074bae25134fbd61c2bfda0533`
...
- The actual paired rollout pilot uses only **five** source snippets, two
  ...
  contains 40 chains but is bound to the old V8 source. The current corrected-
  V11 run is the first V10-bound pilot. It finished during this audit with 40
  chains, although the owner status cross-check still rejects its terminal
  evidence.

The user's view-jitter diagnosis is confirmed. The writer explicitly sets
...
deterministic target look-at. This is a systematic support restriction, not a
sampling accident. It also violates the accepted seminar-generation precedent:
...
`view_roll_jitter_deg = 0.0`. The rollout contract must adopt those exact
effective values. “View jitter must never be zero” means nonzero azimuth and
elevation; zero roll is intentional for the 5-DoF action space.
...
3. Restore the final seminar view-jitter contract—`60°` azimuth, `30°`
   ...
   azimuth or zero-elevation profile is invalid, not an experimental baseline.
...
and V11 the first V10-bound destination. This provenance correction is
consequential: V10 is historical comparison evidence; V11 is the current
source-bound pilot but is not launch-ready while its terminal status evidence
fails closed.
...
1. **Zero view jitter — implementation fact and contract violation.** The
   writer sets all four view caps to `0.0`, including component overrides.
   ...
   preflight must reject an effective azimuth or elevation cap of zero.
...
| [#56 reject legacy stores](https://github.com/JanDuchscherer104/ARIA-NBV/issues/56) | Explains why completed V8-bound V10 shards cannot prove current V11 readiness |
...
1. Make `nbv-rollout-campaign status` accept V11's exact terminal evidence
...
2. **Mandatory seminar view jitter.** Set the resolved base and component
   ...
   effective azimuth or elevation. Empirical AEA/ASE residuals may later
...
- **View jitter:** freeze the final seminar values for every arm:
  ...
  values; any zero azimuth/elevation override fails benchmark construction.
...
| [Gonzalez farthest-first traversal](https://doi.org/10.1016/0304-3975(85)90224-5) | A simple k-center diversity selector after hard filtering | Requires a justified normalized task-pose metric |
...
- Config/source links above open the exact local checkout at audited commit
  `52e9d262…`. That local commit was not published on GitHub at link-check time,
  so the report does not manufacture dead SHA-pinned blob URLs; issue links are
  live GitHub URLs.
...
1. V11 terminal evidence is accepted by the campaign status owner.
...
4. Every resolved rollout component uses the final seminar view-jitter contract
   (`60°` azimuth, `30°` elevation, `0°` roll); preflight rejects zero effective
   azimuth/elevation.
5. #68/#69/#71 eliminate history-independent tables, deterministic gaze
...
7. #81 freezes evaluation scenes/population separately from enriched training
   acquisition.

...
## Accepted 2026-08-21 Native-Minimal Frontmatter And Progressive Disclosure Amendment

The native-minimal frontmatter and progressive-disclosure migration is recorded
as the accepted target-state implementation for this scaffold lane. The 11
ARIA-owned custom skills now expose only `name` and `description` in their
frontmatter. Routing registries were removed from frontmatter and relocated to
compositional skill bodies, conditional references, and outcome-oriented
routing fixtures. Bodies retain only invocation-wide invariants and branch
selectors; branch-specific procedures and inventories are disclosed from their
own references.

`aria-nbv-context/references/context7_library_ids.md` is the single registry
owner for exact Context7 library IDs and Context7 plugin call identifiers.
Consumers route to that reference when external API or version evidence is
needed; local owner lookup does not load it. The upstream Graphify bundle
remains explicitly exempt and byte-identical. ARIA-specific Graphify freshness,
provenance, and fallback rules remain behind the progressive-disclosure

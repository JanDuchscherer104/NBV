...
| S3 | diagnostic-route; reproducer; semantic-owner; exact-evidence; no-implementation; verification | `.agents/skills/diagnose-aria/SKILL.md`; `aria_nbv/aria_nbv/rollouts/AGENTS.md`; `aria_nbv/aria_nbv/rri_metrics/AGENTS.md`; `aria_nbv/aria_nbv/rollouts/replay/`; `aria_nbv/tests/rollouts/` | `cd aria_nbv && uv run pytest tests/rollouts -q` | guessed patch; architecture/planning route; implementation edit |
| S4 | direct-search; implementation-owner; tests-opened; optional-unavailable-continued; exact-source-authority; no-hidden-dependency | `.agents/references/source_order.md`; `aria_nbv/aria_nbv/rendering/plotting.py`; `aria_nbv/tests/vin/test_vin_plotting_v3.py`; `aria_nbv/aria_nbv/vin/AGENTS.md` | `rg -n "candidate.*frustum|frustum.*candidate" aria_nbv/aria_nbv aria_nbv/tests` | plugin/network required; cache/retrieval establishes truth; task blocks |

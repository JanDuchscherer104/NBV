...
sources remain authoritative. The ARIA wrapper in `aria-nbv-context` owns
eligibility, projection, and freshness status through
`scripts/check_graphify_freshness.py --json`: query `fresh` or `usable-stale`
artifacts first; repair or reinitialize `unusable` artifacts before eligible
work. If repair cannot restore usability, report the degraded state and use
exact sources rather than treating Graphify as optional.

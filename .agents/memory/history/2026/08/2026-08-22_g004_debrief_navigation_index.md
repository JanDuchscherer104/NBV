...
full Git OID, branch/detached state, and primary/linked worktree provenance.
...
them. Git branch provenance uses YAML-safe JSON string syntax for every valid
branch name, and the canonical frontmatter parser decodes those JSON string
escapes while retaining its legacy scalar fallback. Queries validate index
freshness and source containment before opening Markdown. The index remains
navigation-only and historical Markdown remains evidence.

## Verification
- `aria_nbv/.venv/bin/python -m pytest -q scripts/tests/test_debrief_index.py scripts/tests/test_validate_agent_memory_threads.py scripts/tests/test_validate_agent_memory_retired.py scripts/tests/test_agent_governance_g002.py scripts/tests/test_ownership_consolidation_contract.py scripts/tests/test_ci_impact.py` — 92 passed, 36 subtests passed.
...
- `aria_nbv/.venv/bin/ruff format --check ... && aria_nbv/.venv/bin/ruff check ...` — six changed Python files formatted and lint-clean.

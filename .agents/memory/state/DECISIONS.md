...
- BM25 (k1=1.5, b=0.75) replaces naive term-frequency as the lexical scorer. Porter stemming + a curated `[synonyms]` table in `.configs/infrastructure/litkg.toml` + Levenshtein distance-≤2 fuzzy fallback for zero-exact-hits round out the lexical layer.

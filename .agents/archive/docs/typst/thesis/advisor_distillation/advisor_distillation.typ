...
#symb.entity.endpoint_gain is the #emph[primary fixed-horizon endpoint metric], #symb.entity.return_h is the #emph[rollout training return], and #symb.entity.log_gain is only an algebraic #emph[scale-sensitivity ablation]. The default immediate rollout / #symb.rl.qh reward is root-normalized by #symb.entity.target_error_0, so with $gamma=1$ its additive return telescopes to endpoint gain up to epsilon. The current-error denominator defines state-relative one-step #RRI for diagnostics and VIN compatibility only.

...
- Cross-check claims against code, active thesis, package owners, and roadmap before
  accepting them.
- Resolved terminology updates `docs/typst/shared/glossary.typ`; other durable
  outcomes update the exact thesis, package, reference, or backlog owner. Do
  not add a parallel root context file or ADR tree as a second source of truth.

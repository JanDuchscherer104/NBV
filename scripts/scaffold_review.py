...
def load_corpus(path: Path) -> list[ReviewItem]:
    ...
    for row in _read_jsonl(path):
        ...
        items.append(
            ReviewItem(
                id=str(row.get("cluster_id") or _stable_id("intent", statement)),
                kind="intent",
                theme=str(themes[0]),
...
def load_pr(repo: str, number: int) -> list[ReviewItem]:
    ...
    facts = [
        (
            "PR scope is review-hostile",
            f"PR #{number} changes {pr['changedFiles']} files with +{pr['additions']} / -{pr['deletions']} lines; split it before merge.",
        ...
        (
            ...
            (
                ...
                "The current CI failure is an OMX acceptance clone blocked by a missing Git LFS object, not proof that every PR change is behaviorally broken.",
            ),
        ),
        (
            "PR target",
            f"Review whether “{pr['title']}” should survive as smaller source-owned changes rather than one scaffold replacement.",
            (f"Base: {pr['baseRefName']}",),
        ),
        (
            "Recover the thesis prune audit",
            "Extract the deterministic thesis prune audit and visible review markers as an independent, source-owned PR.",
            ("This slice has a narrow build interface and focused tests.",),
        ),
        (
            "Recover Graphify traversal",
            "Extract the upstream-native Graphify adapter, source-link bridge, and focused traversal tests without the scaffold lifecycle rewrite.",
            (
                "Keep exact source authoritative and generated graph state outside this review board.",
            ),
        ),
        (
            "Recover shared Typst ownership",
            "Extract shared glossary, notation, equation, and source-link ownership only after separating unrelated thesis rewrites.",
            (
                "One semantic owner is useful; a broad generated rewrite is not required to establish it.",
            ),
        ),
        (
            "Decide LitKG retirement explicitly",
            "Treat LitKG removal as retirement of an active scaffold capability with replacement and literature-preservation evidence, not as unused-code deletion.",
            (
                "Main still routes Make targets, hooks, skills, configuration, and agent guidance through LitKG.",
            ),
        ),
        (
            "Reject the coupled lifecycle bundle",
            "Do not carry the large OMX lifecycle, skill consolidation, Graphify history, and generated-artifact state machine forward as one PR.",
            ("Each owner needs a separate purpose, interface, and proof.",),
        ),
    ]
    return [
        ReviewItem(
...
.actions{{display:grid;grid-template-columns:1fr 1fr 1.25fr;gap:10px;margin-top:22px}} .yes{{color:var(--yes);border-color:var(--yes)}} .no{{color:var(--no);border-color:var(--no)}} .custom{{color:var(--gold);border-color:var(--gold)}}
...
el.export.onclick=()=>{{const payload={{schema_version:1,dataset_digest:DIGEST,exported_at:new Date().toISOString(),decisions}};const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{{type:'application/json'}}));a.download=`scaffold-decisions-${{DIGEST}}.json`;a.click();URL.revokeObjectURL(a.href)}};
...
def build(args: argparse.Namespace) -> int:
    ...
    (args.output / "index.html").write_text(_render_html(items), encoding="utf-8")
    (args.output / "review.qmd").write_text(_render_qmd(len(items)), encoding="utf-8")
    manifest = {
        "schema_version": 1,
    ...
    print(f"wrote {len(items)} review items to {args.output / 'index.html'}")
    return 0
...
def summarize(args: argparse.Namespace) -> int:
    ...
    for answer in ("yes", "no", "custom"):
        rows = [
            (item_id, row)
            for item_id, row in decisions.items()
            if row.get("answer") == answer
        ]
        print(f"\n## {answer.title()} ({len(rows)})")
        for item_id, row in sorted(rows):

...
class FreshnessTests(unittest.TestCase):
    ...
    def _write_fresh_fixture(self) -> None:
        ...
        (output / "graph.json").write_text(
            json.dumps({"built_at_commit": self.head, "nodes": [{"source_file": "./graphify-input/index.md"}]}),
            encoding="utf-8",
        )
        (output / "cache/stat-index.json").write_text(
            json.dumps({"graphify-input/index.md": {"hashes": {"graphify-input/index.md": digest}}}),
            encoding="utf-8",
        )
    ...
    def test_fresh_json_and_quiet_forms(self) -> None:
        ...
        self.assertEqual(set(json.loads(result.stdout)), {"state", "fresh", "head", "reasons", "next_action"})
    ...
    def test_structural_staleness_predicates_fail_closed(self) -> None:
        cases = {
            "projection": lambda: (self.root / "graphify-input/index.md").write_text("source_revision: wrong\n", encoding="utf-8"),
            "graph": lambda: (self.root / "graphify-out/graph.json").write_text(json.dumps({"built_at_commit": "wrong", "nodes": [{"source_file": "graphify-input/index.md"}]}), encoding="utf-8"),
            "digest": lambda: (self.root / "graphify-out/cache/stat-index.json").write_text(json.dumps({"graphify-input/index.md": {"hashes": {"graphify-input/index.md": "wrong"}}}), encoding="utf-8"),
            "node": lambda: (self.root / "graphify-out/graph.json").write_text(json.dumps({"built_at_commit": self.head, "nodes": [{"source_file": "docs/index.md"}]}), encoding="utf-8"),
        }
    ...
    def test_semantic_marker_wins_over_structural_staleness(self) -> None:
        (self.root / "graphify-out/needs_update").write_text("pending\n", encoding="utf-8")
        (self.root / "graphify-input/index.md").write_text("source_revision: wrong\n", encoding="utf-8")
        result = self._run("--json")
        self.assertEqual(result.returncode, 1)
    ...
    def test_invalid_existing_artifact_beats_another_missing_artifact(self) -> None:
        ...
        self.assertTrue(any("missing stat index" in reason for reason in payload["reasons"]))
    ...
    def test_available_structural_reasons_survive_an_invalid_artifact(self) -> None:
        ...
        payload = self._json()
        self.assertEqual(payload["state"], "invalid")
        self.assertTrue(any("projection source_revision" in reason for reason in payload["reasons"]))

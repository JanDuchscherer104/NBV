...
EXPECTED_MANIFEST_SHA256 = "48b37df12e76090ca2653647438c6339f8a60343a87f82a6f3cae0248807fddc"
EXPECTED_BIBLIOGRAPHY_SHA256 = "813490008843162a5961b397ba5e4586e7e8796cb7fb7cc5a968e70ebb0751ca"
...
def main() -> int:
    ...
    for row in rows:
        ...
        assert any(path.is_file() for path in tex_dir.rglob("*")), f"empty TeX mirror: {tex_dir}"
    public_pages = list((ROOT / "docs/contents/literature").glob("*.qmd"))
    assert len(public_pages) >= 10
    print(f"WP6 literature owners: PASS ({len(rows)} manifest rows, {len(public_pages)} public pages)")
    return 0

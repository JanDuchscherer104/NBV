...
def _initial_selection(
    selected_ids: tuple[str, ...], rubric: Mapping[str, Mapping[str, Any]]
) -> tuple[str, ...]:
    """Close corrected selections over their repository-owned original cases."""
    ...
    initial_ids = [
        trial_id for trial_id in selected_ids if trial_id not in correction_ids
    ]
    for corrected_id in correction_ids:
        source_id = rubric[corrected_id].get("resolution_of")
        if isinstance(source_id, str) and source_id not in initial_ids:

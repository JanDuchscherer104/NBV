...
def _typst_term_macros(terms: list[dict[str, Any]]) -> list[tuple[str, str]]:
    """Return compatibility macro names and their canonical term keys."""

    macros: list[tuple[str, str]] = []
    for term in terms:
        typst_macro = term.get("typst_macro")
        if isinstance(typst_macro, str) and typst_macro:
            macros.append((typst_macro, str(term["id"])))
    return sorted(macros, key=lambda item: item[0].lower())


def _render_typst(terms: list[dict[str, Any]], path: Path) -> None:
    """Render only canonical-backed legacy aliases.

    Glossarium entries and definitions remain owned by ``glossary.typ``.  This
    adapter must never serialize another glossary tuple or definition list.
    """
    ...
    for macro, term_id in _typst_term_macros(terms):
        lines += [
            f"#let {macro} = _canonical-short({_typst_string(term_id)})",
            f"#let {macro}_full = _canonical-long({_typst_string(term_id)})",
        ]
    lines += [
        "",
    ]
...
def _render_compat_yaml(terms: list[dict[str, Any]], path: Path) -> None:
    ...
    header = (
        "# Generated compatibility projection; this file owns no glossary semantics.\n"
        "# Canonical owner: docs/typst/shared/glossary.typ. Run `make glossary` after edits.\n\n"
    )
...
def _render_notation_typst(
    ...
    """Render one Typst list projection per canonical notation registry."""
